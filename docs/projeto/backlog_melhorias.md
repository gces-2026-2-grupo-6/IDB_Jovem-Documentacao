...
| RF02 | Cadastrar e editar líderes atuais pelo painel | C+T | US05 — Alta | Implementação: [@BrzGab](https://github.com/BrzGab), [@SamaraAlvess](https://github.com/SamaraAlvess), [@Joaovitor045](https://github.com/Joaovitor045)<br>Back-end: [@Filipe-002](https://github.com/Filipe-002) (dados), [@JpRodrigues2](https://github.com/JpRodrigues2) (API, segurança e qualidade) | **Implementação:** tela "Diretores & Líderes" no painel, restrita à superadministradora — o administrador comum não vê o item no menu e é redirecionado ao acessar a rota. Listagem em abas (atuais e anteriores), cadastro, edição e exclusão com confirmação. Campos: nome, cargo, foto (link do Drive com pré-visualização), região, redes sociais, mini-biografia, ordem de exibição e marcação manual de "diretor anterior". A seção "Nosso Organograma" da página inicial passou a ler os líderes da API: deixaram de estar fixos no código-fonte. 12 testes E2E. [PR #2](https://github.com/gces-2026-2-grupo-6/IDB_Jovem-Teen/pull/2).<br>**Back-end:** colunas `regiao`, `mini_biografia` e `redes_sociais` (JSON) na tabela `lider`, com migration e listagens de líderes atuais e de diretores anteriores ordenadas por `ordem`. A API passou a aceitar e expor os três campos, com `redes_sociais` como objeto `{"rede": "link"}`, e ganhou as rotas públicas `GET /lider/atuais` e `GET /lider/diretores-anteriores`. Criação, edição e exclusão ficaram restritas a `superadmin` — `admin` e papéis de setor recebem `403` —, e os erros `401`, `403` e `404` foram padronizados e declarados no OpenAPI. 82 testes automatizados do módulo e cobertura de 100% em `src/lider/`. Contrato para o front-end em [Contrato da API de Líderes](contrato-api-lideres.md).<br>**Pendência no front-end:** enviar e ler `mini_biografia` no lugar de `bio` e `redes_sociais` como objeto em vez de texto. O período de gestão (`gestao`) não é persistido e ficou fora do escopo. |
...
| `1.7` | 14/09/2026 | Registro da entrega do back-end da US05 no RF02 — campos de perfil, restrição a superadministradora, erros padronizados e testes | [João Pedro Rodrigues](https://github.com/JpRodrigues2) |
